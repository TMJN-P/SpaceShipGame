...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
	...
	if (DxLib_Init() == -1)
		return -1;
	SpacePicture = LoadGraph("‰æ‘œ\\‰F’ˆ.png");
	SpaceShipPicture = LoadGraph("‰æ‘œ\\’…—¤‘D.png");
	SpaceShipBrokenPicture = LoadGraph("‰æ‘œ\\”š”­.png");
	TitlePicture = LoadGraph("‰æ‘œ\\ƒ^ƒCƒgƒ‹.png");
	HowToPlayPicture = LoadGraph("‰æ‘œ\\‚ ‚»‚Ñ‚©‚½.png");
	Init();
	while (ProcessMessage() == 0) {
		...
		switch(ScreenState) {
			...
			case 2:
				...
				SetFontSize(70);
				DrawString(180, 120, "‚©‚ñ‚½‚ñ", GetColor(255, 255, 255));
				DrawString(210, 270, "‚Ó‚Â‚¤", GetColor(255, 255, 255));
				DrawString(150, 420, "‚Þ‚¸‚©‚µ‚¢", GetColor(255, 255, 255));
				if (CursorState == 0) {
